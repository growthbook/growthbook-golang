...
func TestRegexCond(t *testing.T) {
	...
	require.True(t, c.Eval(value.New("some test string"), nil, nil))
	require.False(t, c.Eval(value.New("some string"), nil, nil))
}
...
func TestRegexiCond(t *testing.T) {
	...
	t.Run("non-string values", func(t *testing.T) {
		rx := regexp.MustCompile("(?i)test")
		var c Condition = NewRegexiCond(rx)
		require.False(t, c.Eval(value.New(123), nil, nil))
		require.False(t, c.Eval(value.New(true), nil, nil))
		require.False(t, c.Eval(value.New(nil), nil, nil))
	})
}
