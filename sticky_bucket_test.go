...
func TestStickyBucketInExperimentEvaluation(t *testing.T) {
	...
	// 	Variations: []FeatureValue{"control", "treatment"},
	// 	Meta: []VariationMeta{
	// 		{Key: "0"}, // Use "0" as the key to match the variation ID as string
	// 		{Key: "1"}, // Use "1" as the key to match the variation ID as string
	// 	},
	// 	// Force a specific variation by setting weights
	// 	Weights:       []float64{1.0, 0.0}, // 100% chance of variation 0
	// 	HashAttribute: "id",                // Explicitly set hash attribute
	// }
...
func TestStickyBucketCaching(t *testing.T) {
	...
	exp := &Experiment{
		Key:        "cache-test",
		Variations: []FeatureValue{"control", "treatment"},
		Meta:       []VariationMeta{{Key: "control"}, {Key: "treatment"}},
		Weights:    []float64{1.0, 0.0}, // 100% chance of variation 0
	}
	...
	// Ensure we start with a fresh mock counter
	mockCalls = 0

	// First run - this will call the service to check for assignments
	// but won't find any, then will create a new assignment
	...
	firstCallCount := mockCalls
	require.Equal(t, 1, firstCallCount) // Should make one call to check for existing assignments

	// Reset counter
	mockCalls = 0

	// Second run - should use the client's cached assignments, not call service
	result2 := client.RunExperiment(ctx, exp)
	require.True(t, result2.InExperiment)
	require.Equal(t, 0, result2.VariationId)
	require.Equal(t, 0, mockCalls) // No calls to service

	// Create a second experiment with the same hash attribute
	exp2 := &Experiment{
		Key:           "cache-test2",
		Variations:    []FeatureValue{"a", "b"},
		Meta:          []VariationMeta{{Key: "a"}, {Key: "b"}},
		HashAttribute: "id",
		Weights:       []float64{1.0, 0.0}, // 100% chance of variation 0
	}
	...
	// Run second experiment - should reuse cached id attribute document
	result3 := client.RunExperiment(ctx, exp2)
	require.True(t, result3.InExperiment)
	require.Equal(t, 0, result3.VariationId)
	require.Equal(t, 0, mockCalls) // Should use cache for attribute lookup
}
