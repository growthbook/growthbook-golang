...
func TestIsURLTargeted(t *testing.T) {
	t.Run("Returns false when there are no targeting rules", func(t *testing.T) {
	...
	t.Run("handles a mix of include and exclude rules", func(t *testing.T) {
	...
	t.Run("supports an exclude rule on top of an include rule", func(t *testing.T) {
		rules := []URLTarget{
			{
				Include: true,
				Type:    SimpleURLTarget,
				Pattern: "/search",
			},
			{
				Include: false,
