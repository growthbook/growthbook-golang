...
func TestClosingAClientBeforeTheFirstLoadReturnsStopsThePoller(t *testing.T) {
	var requests atomic.Int32

	release := make(chan struct{})

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Only the first load waits: that is what holds Start in flight, where dsStarted is
		// still false and Close cannot reach the data source itself. Later loads answer at
		// once, so a poller that outlived Close shows up as a rising count rather than as a
		// single request parked in this handler.
		if requests.Add(1) == 1 {
			select {
			case <-release:
			case <-r.Context().Done():
			...
			case <-time.After(2 * time.Second):
				// Never block the server's own shutdown, however the test ends.
			}
		}

		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"features":{},"dateUpdated":"2026-01-01T00:00:00Z"}`))
	}))
	defer server.Close()
	...
	time.Sleep(50 * time.Millisecond) // let the first load reach the handler and park there
	...
	require.Equal(t, settled, requests.Load(),
		"the poller kept running after a client closed mid-start")
}
